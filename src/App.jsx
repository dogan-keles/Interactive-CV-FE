...
const MainChat = ({ darkMode, setDarkMode }) => {
  ...
  // Ortam değişkenine göre API URL'ini belirle
  const API_BASE_URL = import.meta.env.VITE_API_URL || 'https://lengthy-sarina-cypralex-fb6a4e7e.koyeb.app';
  const PROFILE_ID = 1;

  // SEO Verisini Çekme (Düzeltildi)
  useEffect(() => {
    const fetchSEO = async () => {
      try {
        // Başına /api ekledik ve await kullanımını async fonksiyon içine aldık
        const response = await fetch(`${API_BASE_URL}/api/profile/${PROFILE_ID}/seo`);
        if (!response.ok) throw new Error('SEO verisi çekilemedi');
  ...
  return (
    <>
      {seoData && (
        <SEO
          title={`${seoData.title} | Interactive CV`}
